...
def Temperature():
    ...
    Args:
        sub_choice (int): choice from main options
    """
    print("Following are the available temperature conversions.\n____________________________\n 1.Celsius to farenheit \n 2.Farenheit to celsius")
    sub_choice=int(input("Choose an option from above\n(Your choice has to be the numbers corresponding to the options.):"))
                    
    quantity=float(input("Enter the temperature:"))

    if sub_choice==1:
        print(str(quantity)+ " celsius is "+ str((quantity*(9/5))+32) + " farenheit")
    elif sub_choice==2:
        print(str(quantity)+ " farenheit is "+ str((quantity-32)*(5/9)) + " celsius")
    else:
        print("Invalid input.")
